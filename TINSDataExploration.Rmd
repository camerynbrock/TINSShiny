...
nethours_seasonal <- nethours_daily %>% 
  mutate(month = month(date)) %>% 
  filter(month %in% c(8, 9, 10)) %>% 
...
tins_seasonal <- tins_roi %>% 
  mutate(week = lubridate::isoweek(date)) %>% # Used isoweek() instead of week() because it matched with the weekyear used from tsibble
  group_by(family, family_label, spec, spec_label, week) %>% 
  summarize(count = n()) %>% 
  inner_join(nethours_seasonal, by = "week") %>% 
  mutate(count_per_hour = count / nethours)
...
ggplot(tins_seasonal, aes(x = week,
                          y = count_per_hour)) + 
  geom_point(aes(color = family),
             alpha = 0.75,
             show.legend = FALSE) +
...
# Should be count per net hour - change
