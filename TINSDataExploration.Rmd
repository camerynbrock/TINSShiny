...
tins_roi <- tins_tidy %>% 
  ...
  select(family, family_label, spec, spec_label, date, loc, year_month, year, month, doy, note_2)

tins_nethours <- tins_roi %>% 
  group_by(family, family_label, spec, spec_label, year, year_month) %>% 
  summarize(count = n())

nethours_monthly <- nethours_daily %>% 
